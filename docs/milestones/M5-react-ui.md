...
| Model 与 Approval 放入输入器工具栏 | 两者直接影响下一次提交，和输入动作放在同一控制面更容易理解 | 固定在全局状态栏：离任务输入较远，而且容易被误认为全局设置 |
...
10. 把会话级 Model 与 Approval 选择器从底部状态栏迁入输入器工具栏；运行期间跟随输入器锁定，底栏只保留 workspace 状态。实图复查后移除会换行的常驻快捷键提示。
